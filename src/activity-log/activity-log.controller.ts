...
export class ActivityLogController {
  ...
  async getChartData(
    ...
    return this.activityLogService.getChartData(
      groupBy || 'hour',
      startDate,
      endDate,
      city,
  ...
  async getFailures(@Param('id') id: string) {
    const failures = await this.activityLogService.getFailuresByLogId(
      Number(id),
    );
    return failures;
  }
  ...
  async exportFailures(@Param('id') id: string, @Res() res: Response) {
    const buffer = await this.activityLogService.exportFailuresExcel(
      Number(id),
    );
