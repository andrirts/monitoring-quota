...
export class ActivityLogService {
  ...
  async flagAndSnapshot(
    ...
    const statusHabis = allData.filter(
      (d) => d.kuota < this.EXHAUSTED_THRESHOLD,
    ).length;
  ...
  async getChartData(
    ...
    const cities = city ? [city] : Array.from(allCities).sort();
    ...
    for (const log of logs) {
      if (log.cityBreakdown.length > 0) {
        for (const cb of log.cityBreakdown) {
          ...
          cityTotals.set(
            cb.city,
            (cityTotals.get(cb.city) || 0) + cb.newlyExhausted,
          );
        }
        ...
        cityTotals.set(
          'All Location',
          (cityTotals.get('All Location') || 0) + log.newlyExhausted,
        );
      }
    }
