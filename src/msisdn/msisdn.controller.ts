...
export class MsisdnController {
        ...
        if (
          ...
          cb(
            new BadRequestException(
              'Format file tidak didukung. Gunakan .xlsx atau .csv',
            ),
            false,
          );
        }
  ...
  async bulkUpload(@UploadedFile() file: Express.Multer.File) {
    if (!file) {
      throw new BadRequestException(
        'File tidak ditemukan. Upload file Excel atau CSV.',
      );
    }
    ...
    if (parsedData.length === 0) {
      throw new BadRequestException(
        'File kosong atau format kolom tidak sesuai.',
      );
    }
    ...
    FileInterceptor('file', {
      ...
      fileFilter: (_req, file, cb) => {
        ...
        if (
          ...
          cb(
            new BadRequestException(
              'Format file tidak didukung. Gunakan .xlsx atau .xls',
            ),
            false,
          );
        }
  ...
  async bulkRemove(@UploadedFile() file: Express.Multer.File) {
    if (!file) {
      throw new BadRequestException(
        'File tidak ditemukan. Upload file Excel (.xlsx).',
      );
    }
    ...
    if (msisdnList.length === 0) {
      throw new BadRequestException(
        'File kosong atau tidak ada MSISDN yang ditemukan di kolom B.',
      );
    }
  ...
  async findAll(
    ...
    return this.msisdnService.findAll(
      Number(page),
      Number(limit),
      status,
      search,
