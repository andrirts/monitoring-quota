...
export class MsisdnService {
    ...
    return new Promise((resolve, reject) => {
      ...
      stream
        ...
        .on('data', (row: Record<string, string>) => {
          ...
          const usedQuota =
            Number(
              row['Used Quota'] || row['usedQuota'] || row['used_quota'],
            ) || 0;
  ...
  async bulkUpload(data: BulkUploadItemDto[]) {
    ...
    if (newData.length === 0) {
      this.logger.log(
        `Bulk upload: semua ${data.length} data sudah ada di DB, tidak ada yang diupload`,
      );
      return {
        count: 0,
    ...
    this.logger.log(
      `Bulk uploaded ${result.count} records, skipped ${skippedCount} duplicates`,
    );
    return {
      count: result.count,
      skipped: skippedCount,
  ...
  async exportToExcel(data?: any[]): Promise<Buffer> {
    const records =
      data ||
      (await this.prisma.client.msisdn.findMany({ orderBy: { id: 'asc' } }));
